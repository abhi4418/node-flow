...
function UploadVideoNodeComponent({ id, data }: NodeProps) {
  ...
  useEffect(() => {
    ...
    uppyInstance.use(Transloadit, {
      assemblyOptions: async () => {
        const response = await fetch("/api/transloadit/signature/video");
        const data = await response.json();
        return { params: data.params, signature: data.signature };
      },
    });
    ...
    uppyInstance.on("complete", (result) => {
      ...
      if (result.successful && result.successful.length > 0) {
        ...
        const transloaditResult = file.response?.body?.results as
          | Record<string, unknown[]>
          | undefined;
        const exportedFiles = (
          transloaditResult?.exported || transloaditResult?.[":original"]
        ) as Array<{ ssl_url?: string }> | undefined;
        const uploadedUrl = exportedFiles?.[0]?.ssl_url || file.uploadURL;
        if (uploadedUrl) {
          updateNodeData(id, { videoUrl: uploadedUrl, fileName: file.name });
  ...
  return (
    <>
      <Handle type="source" position={Position.Right} id="video-output" isConnectable={true} />
        ...
        <div className="space-y-3">
          {nodeData.videoUrl ? (
            <div className="relative">
              ...
              <p className="mt-1 truncate text-xs text-muted-foreground">{nodeData.fileName}</p>
            </div>
          ) : (
            <div className="space-y-2">
              ...
              <div className="flex flex-col items-center justify-center rounded-md border-2 border-dashed border-border p-4 hover:border-primary/50 transition-colors">
                ...
                <p className="text-xs text-muted-foreground mb-2">MP4, MOV, WebM, M4V (max 100MB)</p>
