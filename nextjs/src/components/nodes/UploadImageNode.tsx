...
function UploadImageNodeComponent({ id, data }: NodeProps) {
  ...
  useEffect(() => {
    ...
    uppyInstance.use(Transloadit, {
      assemblyOptions: async () => {
        const response = await fetch("/api/transloadit/signature/image");
    ...
    uppyInstance.on("complete", (result) => {
      ...
      if (result.successful && result.successful.length > 0) {
        ...
        if (uploadedUrl) {
          updateNodeData(id, { imageUrl: uploadedUrl, fileName: file.name });
  ...
  return (
    <>
      <Handle type="source" position={Position.Right} id="image-output" isConnectable={true} />
          ...
          {nodeData.imageUrl ? (
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-col items-center justify-center rounded-md border-2 border-dashed border-border p-4 hover:border-primary/50 transition-colors">
                ...
                <p className="text-xs text-muted-foreground mb-2">JPG, PNG, WebP, GIF (max 10MB)</p>
